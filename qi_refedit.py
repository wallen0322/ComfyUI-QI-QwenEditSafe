...
def _resize_bchw_smart(x:torch.Tensor,w:int,h:int)->torch.Tensor:
    _,_,H,W = x.shape; return _resize_bchw(x,w,h,_choose_method(W,H,w,h))
...
def _hf_ref_smart(bhwc: torch.Tensor, alpha: float, blur_k:int, kstd: float, bias: float, smooth_k:int)->torch.Tensor:
    ...
    cap=da.mean(dim=(2,3),keepdim=True)+2.3*da.std(dim=(2,3),keepdim=True)
    ...
    out=(bchw+alpha*(dshrink*gate3)).clamp(0,1)
...
class _Blk:
    __slots__=("rng","lat_w","pixE_w","pixM_w","pixL_w","hf_rng")
    def __init__(self,rng,lat,pE,pM,pL,hf=None): self.rng=rng; self.lat_w=lat; self.pixE_w=pE; self.pixM_w=pM; self.pixL_w=pL; self.hf_rng=hf if hf is not None else rng

def _three_blocks(mode:str):
    tA,tB=0.44,0.56
    if mode=="fast":
        e=_Blk(_rng(0.00,tA),0.58,0.28,0.30,0.00,_rng(0.94,1.00))
        m=_Blk(_rng(tA,tB),0.78,0.42,0.58,0.05,_rng(0.94,1.00))
        l=_Blk(_rng(tB,1.00),0.90,0.58,0.80,0.07,_rng(0.94,1.00))
        hp=(0.72,0.004,0.23,3)
    elif mode=="best":
        e=_Blk(_rng(0.00,tA),0.62,0.34,0.36,0.00,_rng(0.96,1.00))
        m=_Blk(_rng(tA,tB),0.92,0.50,0.78,0.06,_rng(0.96,1.00))
        l=_Blk(_rng(tB,1.00),1.02,0.62,0.90,0.08,_rng(0.96,1.00))
        hp=(0.74,0.0045,0.22,3)
    elif mode=="balanced":
        e=_Blk(_rng(0.00,tA),0.62,0.30,0.34,0.00,_rng(0.94,1.00))
        m=_Blk(_rng(tA,tB),0.85,0.46,0.66,0.06,_rng(0.94,1.00))
        l=_Blk(_rng(tB,1.00),0.98,0.60,0.88,0.08,_rng(0.94,1.00))
        hp=(0.72,0.004,0.23,3)
    else: # natural tuned
        e=_Blk(_rng(0.00,tA),0.60,0.30,0.34,0.00,_rng(0.97,1.00))
        m=_Blk(_rng(tA,tB),0.90,0.48,0.72,0.06,_rng(0.97,1.00))
        l=_Blk(_rng(tB,1.00),1.02,0.60,0.92,0.05,_rng(0.97,1.00))
        hp=(0.80,0.0048,0.20,3)
    return e,m,l,hp

# === Node ===
class QI_RefEditEncode_Safe:
    CATEGORY = "QI一致性编辑（安全版）| QI by wallen0322"
    ...
    def encode(self, clip, prompt, image, vae, quality_mode="natural"):
        pad_mode="reflect"; grid_multiple=64
        inject_mode="both"; encode_fp32=False; vl_max_pixels=16_777_216
        no_resize_pad=True

        src=_bhwc(image)[...,:3]
        H,W=int(src.shape[1]),int(src.shape[2])
        gm=max(8,int(grid_multiple)); 
        if gm%self._ALIGN_MULTIPLE!=0: gm=_ceil_to(gm,self._ALIGN_MULTIPLE)
        ...
        if encode_fp32 and isinstance(lat,torch.Tensor) and lat.dtype!=torch.float32: lat=lat.float()

        # 依据模式决定是否生成像素参考，避免冗余计算
        early,mid,late,(kstd,bias,alpha,smooth_k)=_three_blocks(quality_mode)
        need_pixels = inject_mode in ("both","pixels")
        want_pixE = need_pixels and (early.pixE_w>0)
        want_pixM = need_pixels and (early.pixM_w>0 or mid.pixM_w>0 or late.pixM_w>0)
        want_pixL = need_pixels and (late.pixL_w>0)

        pix_base = padded if need_pixels else None
        pixE = _lowpass_ref(pix_base,64) if want_pixE else None
        pixM = pix_base if want_pixM else None
        pixL = _hf_ref_smart(pix_base,alpha,3,kstd,bias,smooth_k) if want_pixL else None

        def add_blk(c, blk, useE, useM, useL):
            c=node_helpers.conditioning_set_values(c,{
                "reference_latents":[lat],
                "strength": float(blk.lat_w),
                "timestep_percent_range":[float(blk.rng[0]), float(blk.rng[1])],
            }, append=True)
            if useE and pixE is not None and blk.pixE_w>0:
                c=node_helpers.conditioning_set_values(c,{
                    "reference_pixels":[pixE],
                    "strength": float(blk.pixE_w),
                    "timestep_percent_range":[float(blk.rng[0]), float(blk.rng[1])],
                }, append=True)
            if useM and pixM is not None and blk.pixM_w>0:
                c=node_helpers.conditioning_set_values(c,{
                    "reference_pixels":[pixM],
                    "strength": float(blk.pixM_w),
                    "timestep_percent_range":[float(blk.rng[0]), float(blk.rng[1])],
                }, append=True)
            if useL and pixL is not None and blk.pixL_w>0:
                c=node_helpers.conditioning_set_values(c,{
                    "reference_pixels":[pixL],
                    "strength": float(blk.pixL_w),
                    "timestep_percent_range":[float(blk.hf_rng[0]), float(blk.hf_rng[1])],
                }, append=True)
            return c

        with torch.inference_mode():
            cond=add_blk(cond, early, useE=True,  useM=True,  useL=False)
            cond=add_blk(cond, mid,   useE=False, useM=True,  useL=False)
            cond=add_blk(cond, late,  useE=False, useM=True,  useL=True)

        latent={"samples":lat, "qi_pad":{"top":int(top),"bottom":int(bottom),"left":int(left),"right":int(right),"orig_h":int(H),"orig_w":int(W)}}
